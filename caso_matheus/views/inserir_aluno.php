...
<form method="post" action="processa_aluno.php"> <!-- Vai enviar o formulario pra pag processa_aluno -->
	<br>
...
<?php } else { ?>
	...
	<?php while($linha = mysqli_fetch_array($consulta_alunos)) {  ?>
		<?php if($linha['id_aluno'] == $_GET['editar']) { ?>
			<h1>Editar aluno</h1>
			<form method="post" action="edita_aluno.php"> <!-- Vai enviar o formulario pra pag processa_curso -->
